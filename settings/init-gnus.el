(use-package gnus
  ...
  (use-package sendmail
    ...
    ;; mail-envelope-from will be automatically loaded if sendmail.el is
    ;; loaded, this is in case
  ...
  (use-package nnrss
    :config
    ;; asynchronously RSS feeding by wget
    (setq nnrss-use-local t)
    (setq cc/nnrss-download-script-file "~/bin/nnrsscrawler")

    (defun cc/nnrss-generate-download-script ()
...
instead of `nnrss-group-data'"
	...
	(progn
	  ...
	  (write-file cc/nnrss-download-script-file)
	  (shell-command (concat "chmod u+x " cc/nnrss-download-script-file)))
	)
      )
...
for `content:encoded' tag if not given"
    ...
    (defun my-save-crawler-after-make-rss-group (original-fun &rest args)
      "save newly added group to `cc/nnrss-download-script-file'"
      (let ((old-length (length nnrss-group-alist)))
	(apply original-fun args)
	(if (file-exists-p cc/nnrss-download-script-file)
		...
		(write-region cmd-string nil cc/nnrss-download-script-file 'append)
		)
	      )
	  (cc/nnrss-generate-download-script cc/nnrss-download-script-file)
	  )
	)
      )
  ...
  ;; 		  ))
  ;;   (set-face-attribute face nil :fontset (cc/fontset-han-twice-en-width "gnus"))
  ;;   )

  (defun cc/fontset-for-gnus-summary-faces (&optional frame)
    (if frame
      (with-selected-frame frame
	(when (display-graphic-p)
	  (let ((fset (cc/fontset-han-twice-width))
		(faces '(gnus-summary-normal-read
  			 gnus-summary-normal-ancient
  			 gnus-summary-normal-undownloaded
  			 gnus-summary-normal-ticked
  			 gnus-summary-normal-unread
	    ...
	    (dolist (face faces)
	      (set-face-attribute face nil :fontset fset))
	    ))
	)
      (when (display-graphic-p)
	(let ((fset (cc/fontset-han-twice-width))
  ...
  (add-hook 'after-make-frame-functions #'cc/fontset-for-gnus-summary-faces)
  (add-hook 'window-setup-hook #'cc/fontset-for-gnus-summary-faces)
